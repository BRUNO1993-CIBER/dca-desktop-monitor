...
from config.tema_cripto import (
    BG_CARD, BTC_ORANGE, TEXT_PRIMARY, TEXT_SECONDARY, TEXT_MUTED, BORDER
)
...
class DonutChart(ctk.CTkFrame):
    ...
    def _atualizar_tabela(self) -> None:
        ...
        for moeda, d in self._dados:
            ...
            self.tree.insert("", "end",
                             values=(f"■  {moeda}", f"{percentual:.2f}%", qtd_str),
                             tags=(moeda,))
    ...
    def _desenhar(self) -> None:
        ...
        self.canvas.create_line(pad_x, y_base, w - pad_x, y_base,
        ...
        for i, (moeda, d) in enumerate(self._dados):
            ...
            if slot_width >= 25:
                self.canvas.create_text(cx, y2 + 15,
                                        text=moeda,
                                        fill=TEXT_SECONDARY,
